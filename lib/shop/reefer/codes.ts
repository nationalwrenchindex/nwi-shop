// Compile-time reefer alarm catalog. Ported verbatim from NWI Suite
// (src/lib/hd/alarm-codes.ts) so a tech gets a code meaning with no network at
// all — this file is the offline floor of the Reefer Alarm Codes tool.
//
// 374 entries total: 239 TK + 14 TK DSR + 83 Carrier + 38 Carrier pretrip.
// The curated per-code depth (diagnostic steps, book time, parts) lives in the
// shared `hd_alarm_codes` table and is layered on top in lookup.ts.
//
// Do not edit the entries or the disclaimer text without a source document.
