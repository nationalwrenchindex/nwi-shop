// The 19 CVSA vehicle inspection categories and every item under them, ported
// verbatim from the National Wrench Index platform. This list IS the substance of
// a DOT annual inspection — 49 CFR 396.17 and Appendix A to Part 396 — and it is
// the one thing in this feature that must not be paraphrased, reordered or
// trimmed. Edit it only against the regulation.
//
// The safetyCritical flag marks the items that ground the vehicle on failure. The write
// path in ../result.ts refuses an inspection carrying one of those unless the
// inspector has confirmed removal from service.

import type { InspectionFormDef, InspectionItem, InspectionSection } from './types'
...
export const CATEGORY_ITEMS: Record<string, InspectionItem[]> = {
  brake_systems: [
...
/** The 19 categories as inspection sections, so DOT and aerial share one engine. */
export const DOT_SECTIONS: InspectionSection[] = INSPECTION_CATEGORIES.map((cat) => ({
  id:    cat.id,
  num:   cat.num,
  label: cat.label,
...
export const DOT_FORM: InspectionFormDef = {
  type:        'dot',
  cadence:     null,
  title:       'Annual DOT Vehicle Inspection',
  citation:    'FMCSA 49 CFR Part 396 · Appendix A',
  requirement:
    'Required at least once every 12 months on every commercial motor vehicle. ' +
    'Must be performed by a qualified inspector as defined by 49 CFR 396.19.',
