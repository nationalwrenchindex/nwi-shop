// SERVER-ONLY prompt corpus for QuickWrench HD. Ported VERBATIM from
// national_wrench_index:
//   - src/app/api/hd/quickwrench/route.ts      (reefer system prompt, electrical
//     system prompt, search directive, fallback text, Carrier disclaimer)
//   - src/lib/hd/truck-diagnostic.ts           (truck system prompt, search
//     directive, fallback text, truck disclaimer)
//   - src/lib/hd/alarm-codes.ts                (TK_DISCLAIMER string only)
//
// The wording is not decorative. Every safety clause, voltage spec, torque
// figure and part number in here was written by a working HD tech and is
// reproduced character for character. Do NOT paraphrase, "tidy", shorten or
// reorder any of it — a rewritten safety clause on a Class 8 truck is how
// somebody gets hurt. If a spec is wrong, fix it upstream in NWI Suite and
// re-port, so the two products never disagree.
//
// These strings are only ever used as `system` instructions to a model. They
// contain no interpolation and no executable content.
...
export const CARRIER_DISCLAIMER = "Alarm code definitions sourced from publicly available Carrier Transicold operator reference information. Not all codes apply to all unit models. Always verify against official Carrier documentation and consult your company for final decisions."
...
// ─── Truck engine / J1939 (primary QuickWrench HD prompt) ────────────────────
...
// ─── Transport refrigeration (reefer units on HD trailers) ───────────────────

export const REEFER_SYSTEM_PROMPT = `You are an expert heavy duty diesel and transport refrigeration technician with 17 years of field experience servicing Thermo King and Carrier Transicold units, Class 6-8 trucks, and refrigerated trailers. You have deep knowledge of FMCSA regulations, DOT inspection criteria, EPA Section 608 requirements, and service procedures for every major TK and Carrier model.
...
export const REEFER_FALLBACK_ANALYSIS = `ALARM MEANING:
...
export const REEFER_WEB_SEARCH_DIRECTIVE = `You are an expert transport refrigeration diagnostic assistant specializing in Thermo King and Carrier Transicold units. You have access to web search — always search for the specific alarm code and unit model before answering.
