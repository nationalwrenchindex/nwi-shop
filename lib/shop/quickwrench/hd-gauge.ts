// Manifold-gauge pressure diagnostic for transport refrigeration units.
// Ported verbatim from national_wrench_index/src/lib/hd/gauge-diagnostic.ts —
// that module had zero imports and is entirely self-contained, so it lands here
// unchanged apart from this header.
//
// This is a PURE lookup: no network, no model, no database. It is the one part
// of QuickWrench HD that works with no API key of any kind, which is exactly why
// it is surfaced as a standalone panel in the UI.
//
// The pattern matrix, the causes and the labor estimates are field-authored
// content. Do not paraphrase or "improve" the wording — see hd-prompts.ts.
