...
//
// Consumed by app/api/shop/tools/trailer-abs/diagnostic via @/lib/gemini (gemini-3.6-flash).
...
export interface AbsEcuTable {
  ...
  key: 'BENDIX_EC60' | 'BENDIX_EC30' | 'HALDEX_GEN4' | 'HALDEX_GEN5'
...
export const ABS_ECU_TABLES: readonly AbsEcuTable[] = [
  {
    // EC-60 is a BENDIX unit, confirmed by the shop owner against the housings they
    // actually see. It was originally entered here under WABCO; that was wrong, and the
    // consequence of leaving it wrong would have been a tech reading "EC-60" off a
    // housing and being handed the wrong maker's table — where 2-1 points at a
    // different wheel end. Bendix therefore has TWO tables with two different code
    // formats: EC-30 flashes single digits, EC-60 flashes two. The format is what
    // separates them when a tech says only "Bendix".
    key: 'BENDIX_EC60',
    label: 'Bendix EC-60',
    manufacturer: 'Bendix',
 ...
 * to generateText() / generateDiagnostic() in @/lib/gemini.
 */
...
Bendix EC-60: rectangular black box, EC-60 stamped on the label, 18 or 26 pin connector
...
${NO_TABLE_TEXT}
If the tech is on one of these, you cannot translate their code. Set clarification_needed true and send them to the blink code decal on the ECU housing or the service literature for the part number stamped on the unit. Do not borrow a table from another unit — a Bendix EC-60 table does not apply to a Bendix TABS-6 Advanced, and a Bendix EC-30 table does not apply to either.

WABCO — READ THIS BEFORE ANSWERING ANY WABCO QUESTION:
You have NO code table for any WABCO unit. The only WABCO ECU you can even identify is the TABS-6, and it has no table here. So whenever the tech says WABCO or Meritor WABCO, you must set clarification_needed true and send them to the decal on the housing. Do NOT translate a WABCO code by reaching for the Bendix EC-60 table because the code format looks similar — EC-60 is a Bendix unit, not a WABCO one, and its numbers mean nothing on a WABCO ECU. If the tech says WABCO but describes a box with EC-60 stamped on it, that is a Bendix ECU and you should say so plainly and ask them to confirm the logo, because the two makers are routinely mixed up in the field.

WHY THE TABLES COLLIDE — THE THING THAT ACTUALLY BITES PEOPLE:
"2-1" on a Haldex Gen 4 is a LEFT FORWARD sensor. Haldex Gen 5 does not use two-digit codes at all. "1-1" is an ECU internal fault on a Bendix EC-60 and a power supply fault on a Haldex Gen 4 — opposite ends of the diagnosis. "4-1" is a pneumatic fault on a Bendix EC-60 and an ECU internal fault on a Haldex Gen 4. Same flashes, different repair, different part. This is why the ECU has to be pinned down before the code is read.

BENDIX HAS TWO TABLES AND THEY DO NOT OVERLAP:
Bendix EC-30 flashes SINGLE digits (1 through 6). Bendix EC-60 flashes TWO digits (1-1, 2-3, 4-1). The format is the tell. A tech who says only "Bendix" and gives "2" is on an EC-30; one who says "Bendix" and gives "2-3" is on an EC-60. If the code format does not settle it, ask which unit is on the trailer rather than picking one. Never read a single-digit code off the EC-60 table or a two-digit code off the EC-30 table.
...
F. The manufacturer is WABCO or Meritor WABCO. You have no WABCO table at all — see the WABCO section above. This is not a partial answer situation: you cannot translate any WABCO code. Ask for the decal, and if they describe an EC-60 housing, tell them that is a Bendix unit and ask them to confirm the logo.

G. The manufacturer is Bendix but the unit is not established and the code format does not settle it. EC-30 and EC-60 are both Bendix and both plausible. Ask which is on the trailer.

H. No code has been read yet — the complaint is only "the ABS light is on." Walk them through retrieving the code and ask them for it.

I. The tech gives a code that could belong to more than one ECU they might have, and nothing in their message settles which.
